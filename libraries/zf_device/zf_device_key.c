...
{
    uint8 i = 0;
    uint32 debounce_count = (KEY_MAX_SHOCK_PERIOD + scanner_period - 1U) / scanner_period;
    uint32 long_press_count = (KEY_LONG_PRESS_PERIOD + scanner_period - 1U) / scanner_period;
    ...
    for(i = 0; KEY_NUMBER > i; i ++)
    {
        ...
        {
            key_press_time[i] ++;
            if(long_press_count <= key_press_time[i])
            {
        ...
        else                                                                    // 按键释放
        {
            if((KEY_LONG_PRESS != key_state[i]) &&
               (debounce_count <= key_press_time[i]) &&
               (long_press_count > key_press_time[i]))
            {
