...
void small_driver_set_speed(int16 left_speed, int16 right_speed)
{
    ...
    send_data_buffer[0] = 0xA5;

    send_data_buffer[1] = 0x04;

    send_data_buffer[2] = (uint8)((left_speed & 0xFF00) >> 8);

    send_data_buffer[3] = (uint8)(left_speed & 0x00FF);

    send_data_buffer[4] = (uint8)((right_speed & 0xFF00) >> 8);

    send_data_buffer[5] = (uint8)(right_speed & 0x00FF);

    send_data_buffer[6] = 0;

    for(int i = 0; i < 6; i ++)
    {
        send_data_buffer[6] += send_data_buffer[i];
    }

    interrupt_state = disableInterrupts();
    uart_write_buffer(SMALL_DRIVER_UART, send_data_buffer, 7);
